# Dotfiles

My current dotfiles that take into account the macbook notch.

<img width="1512" alt="image" src="https://github.com/justyeethan/dotfiles/assets/61468547/0ab572d4-56a3-4c24-a761-671a1b7bc606">

<img width="1512" alt="image" src="https://github.com/justyeethan/dotfiles/assets/61468547/5dc86423-e4c6-4eaf-bf2d-62edf53252e0">


To change the theme, switch to the .bak files for sketchybar and the yabairc file.

## Old Config:

This config is alright, but I wanted to save more space by just moving the status bar to the top, which the notch already takes up room.

<img width="1440" alt="image" src="https://github.com/justyeethan/dotfiles/assets/61468547/0ec38b1b-8ca8-44fa-94d1-fadfe0f26f75">

<img width="1440" alt="image" src="https://github.com/justyeethan/dotfiles/assets/61468547/b6ac72dd-16dc-4fa0-a220-b0da8a6c848e">

<img width="1440" alt="image" src="https://github.com/justyeethan/dotfiles/assets/61468547/ee22a1ce-e4e8-4b47-88f3-e18eedd30125">

